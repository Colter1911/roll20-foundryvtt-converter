...
function patchActivity(act, row) {
  ...
  // Primary damage
  const dmgF = f(row, "dmgbase", "npc_dmg1", "dmg1");
  const dmgT = f(row, "dmgtype", "npc_dmg1_type", "dmg1type");
  if (dmgF) {
    act.damage ??= { parts: [] };
    act.damage.parts ??= [];
  ...
  // Secondary damage
  const dmg2F = f(row, "dmg2base", "npc_dmg2", "dmg2");
  const dmg2T = f(row, "dmg2type", "npc_dmg2_type", "dmg2type");
  if (dmg2F) {
    act.damage ??= { parts: [] };
    act.damage.parts ??= [];
 ...
 */
export async function enrichItem(row, activationType, isSpell, index, idMapper) {
  const name = isSpell
    ? f(row, "spellname")
    : f(row, "name", "atkname");
  ...
  if (name) {
    const preferredType = isSpell ? "spell" : null;
